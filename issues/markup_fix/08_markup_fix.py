"""
Markup fixer + audit for ben.txt (BEN).
Counterpart of PWG#175, PWK#113, and siblings.

BEN-specific notes: 1 trailing space in <lang>.

Inputs: ../../../csl-orig/v02/ben/ben.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "ben" / "ben.txt", HERE / "ben.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "ben_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ab|ls|lang)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for ben.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"BEN markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against ben.txt.\n")
        f.write(f"Paired tags: <ab>, <ls>, <lang>\n")
        f.write(f"Notes: 1 trailing space in <lang>.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ab>, <ls>, <lang>\n\n")
