 ...
 for rec in recs:
  ...
  for iline,line in enumerate(entry.datalines):
   if iline == 0:
    line = re.sub(r'^.*¦','',line)
   line = line.strip()
   linesanskrits = re.findall(r'{#[^#]+#}',line)  # all sanskrit text this line
   line1 = line # line[0:20] # truncate line. upasargas should occur near beginning
   y = [b[2:-2] for b in linesanskrits]  # remove {# and #}
   for b in y:
    c = b
    if c not in raw_sanskrits:
     raw_sanskrits.append(c)
  upasargas0 = [knownupad[x] for x in raw_sanskrits if x in knownupad]
  upasargas = []
  for u in upasargas0:
...
def prelim(fileout,recs):
 ...
 for rec in recs:
  ...
  L = entry.metad['L']
  k1 = entry.metad['k1'] 
  sanskrits = [] # all text marked as sanskrit 
  outarr = [entry.metaline]
  for iline,line in enumerate(entry.datalines):
   status = '+' # prob. ok.
   if iline == 0:
   ...
   outarr.append(out)
   # update raw_sanskrits
   for b in y:
    c = b
    if c not in raw_sanskrits:
     raw_sanskrits[c] = 0
    raw_sanskrits[c] = raw_sanskrits[c] + 1
  outarr.append('')
  for out in outarr:
   f.write(out+'\n')
 print('preverb0. Debug info written to',fileout)
 print(len(raw_sanskrits),"different sanskrit text found")
 sanskritkeys = sorted(raw_sanskrits.keys())
 for b in sanskritkeys:
  c = re.sub(r'[,:]','',b)
  f.write('%03d\t%s\t%s\n'%(raw_sanskrits[b],b,c)) # two copies 
 f.close()
...
if __name__=="__main__": 
 ...
 filein1 = sys.argv[2] # ben_verb_filter.txt
 filein2 = sys.argv[3]  # ben_upasarga_map.txt
 fileout = sys.argv[4] # 
